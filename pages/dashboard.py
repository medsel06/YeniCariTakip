        ...
        with ui.dialog() as _wn_dlg, ui.card().classes('q-pa-none').style(
            ...
            with ui.row().classes('w-full justify-end').style('padding:0 22px 18px'):
                def _wn_kapat():
                    ...
                    _wn_dlg.close()
                ui.button('Anladım', on_click=_wn_kapat).props('unelevated color=positive no-caps')
        _wn_dlg.open()
            ...
            with ui.element('div').classes('w-full gap-4 q-mb-md primary-kpis-grid').style('margin-top: 52px;'):
                ...
                with ui.card().classes(f'{card_class} justify-between').style('border-radius: 12px; padding: 16px 18px !important; gap: 4px;'):
                    with ui.row().classes('w-full justify-between items-center no-wrap'):
                        ui.label('Net Kar / Zarar').classes('text-weight-medium').style('font-size: 11px; text-transform: uppercase; letter-spacing: 0.4px; opacity: 0.9;')
                        with ui.element('div').style('background: rgba(255,255,255,0.2); border-radius: 7px; display: flex; align-items: center; justify-content: center; width: 22px; height: 22px;'):
                            ui.icon('trending_up' if is_profit else 'trending_down', color='white', size='14px')
                    ui.label(f'{fmt_para(net_kar)} TL').classes('text-weight-bolder').style('font-size: 16px; font-weight: 800; letter-spacing: -0.5px;')

                # Kasa Bakiyesi Card
                kasa_bakiye = summary['kasa_bakiye']
                with ui.card().classes('gradient-card-kasa justify-between').style('border-radius: 12px; padding: 16px 18px !important; gap: 4px;'):
                    with ui.row().classes('w-full justify-between items-center no-wrap'):
                        ui.label('Kasa Bakiyesi').classes('text-weight-medium').style('font-size: 11px; text-transform: uppercase; letter-spacing: 0.4px; opacity: 0.9;')
                        with ui.element('div').style('background: rgba(255,255,255,0.2); border-radius: 7px; display: flex; align-items: center; justify-content: center; width: 22px; height: 22px;'):
                            ui.icon('account_balance_wallet', color='white', size='14px')
                    ui.label(f'{fmt_para(kasa_bakiye)} TL').classes('text-weight-bolder').style('font-size: 16px; font-weight: 800; letter-spacing: -0.5px;')
                ...
                with ui.card().classes('modern-card justify-between').style('border-radius: 12px; padding: 16px 18px !important; gap: 5px;'):
                    with ui.row().classes('w-full justify-between items-center no-wrap'):
                        ui.label('SATIŞ / ALIŞ').classes('text-weight-bold').style('font-size: 12px; text-transform: uppercase; letter-spacing: 0.4px; color: #64748b;')
                        ui.label(f'HACİM: {hacim_txt}').style('background:#eff6ff;color:#2563eb;border-radius:20px;font-size:9px;font-weight:700;padding:1px 8px;')
                    with ui.row().classes('w-full justify-between items-center no-wrap'):
                        ui.label(f'{fmt_para(satis)} TL').classes('text-weight-bold text-green-6').style('font-size: 13px;')
                        ui.label(f'{fmt_para(alis)} TL').classes('text-weight-bold text-red-6').style('font-size: 13px;')
